...
LRESULT CALLBACK GraphicsEngine::WndProc(HWND hWnd, UINT message, WPARAM wparam, LPARAM lparam) {
    ...
    case WM_PAINT:
    {
        auto startPaintTime = std::chrono::high_resolution_clock::now();
        ...
        *memHDC = CreateCompatibleDC(*hdc);
        ...
        DeleteDC(*memHDC);
        EndPaint(hWnd, &ps);
        auto endPaintTime = std::chrono::high_resolution_clock::now();

        auto paintDelta = endPaintTime - startPaintTime;
        //printf("Milliseconds To Paint: %i\n", std::chrono::duration_cast<std::chrono::milliseconds>(paintDelta).count());

        std::chrono::high_resolution_clock::time_point now = std::chrono::high_resolution_clock::now();
        std::chrono::high_resolution_clock::duration delta = now.time_since_epoch() - graphicsEngine->lastTime.time_since_epoch();
        graphicsEngine->observedFPS = 1000 / (std::chrono::duration_cast<std::chrono::milliseconds>(delta)).count();
        graphicsEngine->lastTime = now;
...
double GraphicsEngine::getObservedFPS() {
    return observedFPS;
}
